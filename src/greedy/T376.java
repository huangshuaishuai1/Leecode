    ...
    public int wiggleMaxLength(int[] nums) {
       ...
       int type;
        ...
        for (int i = 1; i < nums.length; i++) {
           ...
           ArrayList<Integer> after = findAfter(now, i, nums, type);
           int len = after.size();
//           Collections.sort(after);
//           if (type < 0) {
//               now = after.get(0);
//           }else {
               now = after.get(after.size()-1);
//           }
            list.add(now);
           count ++;
           i += len -1;
    ...
    private ArrayList<Integer> findAfter(int head, int index, int[] nums, int type) {
